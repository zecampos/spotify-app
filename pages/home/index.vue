          ...
          <v-expansion-panel-content v-for="(item, index) of playlist" :key="index">
            ...
            <v-card v-for="(track, index) of item.tracks" :key="index">
              <Card
                ...
                :dataL="track.track.album.release_date.substring(0,4)"
              ></Card>
              <v-card-text class="#303030"></v-card-text>
            </v-card>
          </v-expansion-panel-content>
        </v-expansion-panel>
      </v-flex>
      <v-btn @click="modalNew = true" fixed dark fab bottom right color="green">
        <v-icon>add</v-icon>
      </v-btn>
    </v-layout>
    <v-layout>
        ...
        <v-dialog v-model="modalNew" persistent max-width="80%">
          <v-layout align-center row wrap>
            <v-flex xs12 sm12 md12 lg12 xl12>
              <v-text-field class="ml-4 mr-4" v-model="newPlaylist.name" label="Name" required></v-text-field>
              <v-text-field class="ml-4 mr-4" v-model="newPlaylist.description" label="Description"></v-text-field>
            </v-flex>
            <v-flex class="mb-4" xs12 sm12 md12 lg12 xl12>
              <v-btn @click="clearPlaylistForm" color="white" class="black--text ml-4">cancel</v-btn>
              <v-btn @click="createPlaylist" color="green">create</v-btn>
            </v-flex>
          </v-layout>
        </v-dialog>
      </v-flex>
